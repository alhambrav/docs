...
Note that the Site Search REST API ``POST /api/1/site/search/search.json`` rejects request bodies that contain restricted JSON object keys, including nested object and array keys, with an HTTP 400 response. See :ref:`engine-search-restricted-key-patterns` for more information.
